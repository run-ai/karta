...
// directory set to test/conformance, so docs/samples/ is two up. The recorded fixtures
// live alongside this package under fixtures/.
const repoRoot = "../.."
	...
	for _, dir := range dirs {
		...
		t.Run(rel, func(t *testing.T) {
			...
			if fx.SchemaVersion != SchemaVersion {
				t.Fatalf("fixture schema v%d != current v%d; re-record with make record-e2e", fx.SchemaVersion, SchemaVersion)
			}

			sampleBytes, err := os.ReadFile(filepath.Join(repoRoot, fx.KartaFile))
			if err != nil {
				t.Fatalf("read sample %s: %v", fx.KartaFile, err)
			}

			var karta v1alpha1.Karta
			if err := yaml.Unmarshal(sampleBytes, &karta); err != nil {
			...
			if got := CollapseConsecutive(snapStates); !slices.Equal(got, fx.ObservedStates) {
