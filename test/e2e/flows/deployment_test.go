...
var _ = Describe("Deployment (built-in)", Ordered, Label("deployment", "builtin"), func() {
	...
	BeforeAll(func(ctx SpecContext) {
		...
		rec = recorder.New("deployment", "apps-deployment-v1", "../../docs/catalog/apps-deployment-v1.yaml").
			...
			State(kartav1alpha1.RunningStatus, CondReason("Progressing", "NewReplicaSetAvailable")).
			State(kartav1alpha1.FailedStatus, CondFalse("Progressing"))
	})

	It("scaled", func(ctx SpecContext) {
		_, err := rec.Flow("scaled", "flows/testdata/deployment/running.yaml").
	...
	It("initializing", func(ctx SpecContext) {
		_, err := rec.Flow("initializing", "flows/testdata/deployment/initializing.yaml").
	...
	It("failed", func(ctx SpecContext) {
		_, err := rec.Flow("failed", "flows/testdata/deployment/failed.yaml").
