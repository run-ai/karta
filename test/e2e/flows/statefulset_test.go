...
var _ = Describe("StatefulSet (built-in)", Ordered, Label("statefulset", "builtin"), func() {
	...
	BeforeAll(func(ctx SpecContext) {
		...
		rec = recorder.New(cluster, "statefulset", operatorVersion("statefulset"), "apps-statefulset-v1", "../../docs/catalog/apps-statefulset-v1.yaml").
	...
	It("scaled", func(ctx SpecContext) {
		_, err := recorder.NewFlow(rec, "scaled", "testdata/statefulset/running.yaml").
	...
	It("degraded", func(ctx SpecContext) {
		_, err := recorder.NewFlow(rec, "degraded", "testdata/statefulset/degraded.yaml").
