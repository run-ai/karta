...
var _ = Describe("StatefulSet (built-in)", Ordered, Label("statefulset", "builtin"), func() {
	...
	BeforeAll(func(ctx SpecContext) {
		...
		rec = recorder.New("statefulset", "apps-statefulset-v1", "../../docs/catalog/apps-statefulset-v1.yaml").
			State(kartav1alpha1.InitializingStatus, ReplicasInitializing()).
			State(kartav1alpha1.RunningStatus, FullyAvailable()).
			State(kartav1alpha1.DegradedStatus, ReplicasDegraded())
	})
	...
	It("scaled", func(ctx SpecContext) {
		_, err := rec.Flow("scaled", "flows/testdata/statefulset/running.yaml").
	...
	It("degraded", func(ctx SpecContext) {
		_, err := rec.Flow("degraded", "flows/testdata/statefulset/degraded.yaml").
