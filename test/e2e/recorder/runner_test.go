	...
	Describe(wc.Name, Ordered, ginkgoLabels(wc), func() {
		...
		for _, fl := range wc.Flows {
			It(fmt.Sprintf("%s flow reaches %s", fl.Name, fl.DesiredFinalStatus()), func() {
				...
				By("creating the workload in " + testNamespace)
				...
				rec, reached := observeTransitions(wc, fl, obj)
				orderErr := observedOrderErr(fl, rec.order)

				By("recording the run")
				succeeded := reached && orderErr == nil
				writeRecording(wc, fl, rec, succeeded)

				Expect(reached).To(BeTrue(), "did not reach %q: %s", fl.DesiredFinalStatus(), rec.failure)
				Expect(orderErr).To(Succeed())
			})
		}
	})
}
