...
When a subtree (e.g. `pkg/jq/`, `docs/ri-studio/`, `charts/karta/`) has non-obvious local conventions, add a nested `AGENTS.md` (under 200 lines). For Claude Code parity, add a sibling `CLAUDE.md` containing only `@AGENTS.md` (`printf '@AGENTS.md\n' > CLAUDE.md`). Do not use symlinks or put unique content in `CLAUDE.md`.
