...
var _ = Describe("discoverNativeGVKs", func() {
	...
	It("tolerates a partial group discovery failure and uses the reachable groups", func() {
		...
		dc := &fakeDiscovery{
			...
			err: &discovery.ErrGroupDiscoveryFailed{Groups: map[schema.GroupVersion]error{
				failedGV: errors.New("the server is currently unable to handle the request"),
			}},
		}
